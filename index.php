...
//Login DB
define('DSN', 'mysql:host=' . (getenv('DB_HOST') ?: 'localhost') .
              ';dbname=' . (getenv('DB_NAME') ?: 'fmh') .
              ';charset=utf8');
...
define('ROOT_PATH', getenv('ROOT_PATH') ?: "data");
$root_path = ROOT_PATH;
...
// e.g. 'txt,html,css,js'
define('ALLOWED_FILE_EXTENSIONS', getenv('ALLOWED_FILE_EXTENSIONS') ?: '');
$allowed_file_extensions = ALLOWED_FILE_EXTENSIONS;

// Allowed file extensions for upload files
// e.g. 'gif,png,jpg,html,txt'
define('ALLOWED_UPLOAD_EXTENSIONS', getenv('ALLOWED_UPLOAD_EXTENSIONS') ?: '');
...
$external = array(
    ...
    'css-font-awesome' => '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css" crossorigin="anonymous">',
...
if (isset($_COOKIE['theme'])) {
    $theme = $_COOKIE['theme']; // Récupère la valeur du cookie 'theme'
} else {
    $theme = 'light';
}
...
session_start();
session_regenerate_id(true); // Générez un nouvel identifiant de session unique
...
if (isset($_SESSION['jail'])) {
    $wd = fm_clean_path(dirname($_SERVER['PHP_SELF']));
    $root_url =  $root_url . $wd . DIRECTORY_SEPARATOR . $_SESSION['jail'];
}
if (isset($_SESSION['logged']) && !empty($directories_users[$_SESSION['logged']])) {
    $wd = fm_clean_path(dirname($_SERVER['PHP_SELF']));
    $root_url =  $root_url . $wd . DIRECTORY_SEPARATOR . $directories_users[$_SESSION['logged']];
}
...
if (isset($_GET['logout'])) {
    ...
    // Supprimer le fichier de session manuellement
...
if ($use_auth) {
    //Pour l'upload avec curl
    if (isset($_GET['upload_cli'])) {        
        try {
        ...
        // Récupérer le token depuis les en-têtes ou les paramètres GET/POST
        $providedToken = $_SERVER['HTTP_AUTHORIZATION'] ?? ($_POST['token'] ?? ($_GET['token'] ?? null));

        // Vérifier si le token est fourni
        if (!$providedToken) {
            http_response_code(401); // Non autorisé
            echo "Authentification échouée : token manquant.\n";
            exit;
        }

        // Vérifier si le token existe dans la base de données
        $stmt = $pdo->prepare("SELECT * FROM fmh_token WHERE token = :token LIMIT 1");
        $stmt->execute([':token' => $providedToken]);
        $tokenData = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$tokenData) {
            http_response_code(401); // Non autorisé
            echo "Authentification échouée : token invalide.\n";
            exit;
        }else{
            $user_id = $tokenData['user_id'];
        }

        //Récupération du jail d'un utilisateur
        $stmt = $pdo->prepare('SELECT user_id, user_name, user_jail, user_active FROM fmh_users WHERE user_id = :user_id LIMIT 1');
        $stmt->bindParam(':user_id', $user_id, PDO::PARAM_STR);
        $stmt->execute();

        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($user && $user['user_active']==true) {
            if (strlen($user['user_jail']) > 0){
                $root_path = $user['user_jail'];
            }
        } else {
        ...
        // Définir le répertoire où les fichiers seront sauvegardés
        $uploadDir = $root_path;
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0755, true);
        }

        // Si le token est valide, vérifier si un fichier est uploadé
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
            $file = $_FILES['file'];

            if ($file['error'] === UPLOAD_ERR_OK) {
                $targetFile = $uploadDir . DIRECTORY_SEPARATOR . basename($file['name']);

                // Vérifier si le fichier existe déjà
                if (file_exists($targetFile)) {
                    echo "Erreur : Un fichier portant le même nom existe déjà.\n";
                    exit;
                }

                // Déplacer le fichier vers le répertoire cible
                if (move_uploaded_file($file['tmp_name'], $targetFile)) {
                    // Si le fichier est uploadé avec succès, supprimer le token de la base de données
                    $deleteStmt = $pdo->prepare("DELETE FROM fmh_token WHERE token = :token");
                    $deleteStmt->execute([':token' => $providedToken]);
                    
                    echo "Fichier uploadé avec succès vers : $targetFile\n";
                    echo "Le token a été supprimé de la base de données.\n";
                    exit;
                } else {
        ...
        if (isset($_SESSION['logged'])) {
            ...
            try {
                ...
                $stmt = $pdo->prepare('SELECT user_id, user_name, user_pass, user_role, user_jail, user_active FROM fmh_users WHERE user_name = :username LIMIT 1');
                ...
                if ($user) {
                    if ($user['user_active']==true) {
                        if ($user && password_verify($password, $user['user_pass'])) {
                            // Connexion réussie
                            $_SESSION['logged'] = $user['user_name'];
                            if (strlen($user['user_jail']) > 0){
                                $_SESSION['jail'] = $user['user_jail'];
                            }
                            $_SESSION['role'] = $user['user_role'];
                            $_SESSION['user_id'] = $user['user_id'];
                            $_SESSION['db_user'] = True;
                        ...
                        fm_set_msg(lng('Account disabled'), 'error');
                        fm_redirect(FM_SELF_URL);
                    }
                }
            } catch (PDOException $e) {
                ...
                <div class="container h-100">
                    <div class="row justify-content-md-center align-content-center h-100vh">
                        <div class="card-wrapper">
                            <div class="card fat" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                                <div class="card-body">
...
if ($use_auth && isset($_SESSION['logged'])) {
    if (isset($directories_users[$_SESSION['logged']])){
        $root_path = $root_path . DIRECTORY_SEPARATOR . $directories_users[$_SESSION['logged']];
    }elseif (isset($_SESSION['jail'])){
        $root_path = $root_path . DIRECTORY_SEPARATOR . $_SESSION['jail'];
    }
}
...
define('FM_READONLY', 
    (isset($_SESSION['role']) && $_SESSION['role'] === "read_only")
        ? true
        : (
            (isset($_SESSION['role']) && $_SESSION['role'] === "admin")
                ? false 
                : ($global_readonly || ($use_auth && !empty($readonly_users) && isset($_SESSION['logged']) && in_array($_SESSION['logged'], $readonly_users)))
        )
);
...
if ((isset($_SESSION['logged']) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    if (!verifyToken($_POST['token'])) {
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if (is_writable($targetPath)) {
        ...
        if (file_exists($fullPath)) {
            $response = array(
                'status'    => 'error',
                'info'      => "Le fichier ". $fullPathInput ." existe déjà.",
            );
            echo json_encode($response);
            exit();
        }
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    ...
    <div class="path">
        ...
        <div class="card mb-2 fm-upload-wrapper" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" href="#fileUploader" data-target="#fileUploader"><i class="fa fa-circle-up"></i> <?php echo lng('UploadingFiles') ?></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#urlUploader" class="js-url-upload" data-target="#urlUploader"><i class="fa-solid fa-link"></i> <?php echo lng('Upload from URL') ?></a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
...
?>
    <div class="path">
        <div class="card" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            <div class="card-header">
...
?>
    <div class="path">
        ...
        <ul class="folders break-word">
            ...
            ?>
                <li>
                    <a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa-solid fa-folder-open"></i> <?php echo fm_convert_win($f) ?></a>
                </li>
            <?php
            }
            ?>
...
if (isset($_GET['passwd_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    if (!isset($_SESSION['logged'])) {
        header('Location: '.$basename); // Redirige vers une page de connexion si non authentifié
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $user_id = $_SESSION['user_id']; // Supposons que l'ID utilisateur est stocké dans la session
        ...
        if (empty($new_password) || empty($confirm_password)) {
            ...
            try {
                ...
                $update_stmt = $pdo->prepare("UPDATE fmh_users SET user_pass = :new_password WHERE user_id = :user_id");
...
if (isset($_GET['admin_logout_user']) && isset($_GET['id']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    // Vérifiez si l'utilisateur est authentifié
    ...
    // Déconnecter cet utilisateur (en supprimant son fichier de session)
    $session_path = ini_get('session.save_path');
    if (is_dir($session_path)) {
        $files = glob($session_path . '/sess_*'); // Trouver tous les fichiers de session
        ...
        foreach ($files as $file) {
            ...
            if ($data) {
                ...
                if (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $user_id) {
                    unlink($file); // Supprimer le fichier de session
                }

            }
        }

        session_decode($current_session); // Recharger la session courante
    }

    // Redirection après déconnexion
    header('Location: '. $basename .'?p=&admin_user=2');
    exit;
}

if (isset($_GET['admin_logout_all_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    // Vérifiez si l'utilisateur est authentifié
    if (!isset($_SESSION['logged']) || (isset($_SESSION['role']) && $_SESSION['role'] !== "admin")) {
        header('Location: '.$basename); // Redirige vers une page de connexion si non authentifié
        exit;
    ...
    if (is_dir($session_path)) {
        ...
        foreach ($files as $file) {
            ...
            if ($data) {
                ...
                if (isset($_SESSION['user_id']) && $_SESSION['user_id'] != $current_user) {
                    unlink($file); // Supprimer le fichier de session
                }
...
if (isset($_GET['admin_page']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        $pdo = new PDO(DSN, DB_USER, DB_PASSWORD);
        $query = "SELECT user_id, user_name, user_role, user_active, user_jail FROM fmh_users";
    ...
    <div class="dashboard-container">
        <a href="?p=&amp;admin_local_user=2" class="dashboard-card">
            <?php
            // Requête pour compter le nombre de lignes
                echo "<h2>" . count($auth_users) . "</h2>";
            ?>
            <p>Local Users</p>
            <i class="fa-solid fa-users-gear"></i>
        </a>
        <a href="?p=&amp;admin_user=2" class="dashboard-card">
            <?php
            // Requête pour compter le nombre de lignes
            try {
                $stmt = $pdo->query('SELECT COUNT(*) AS row_count FROM fmh_users WHERE user_active=1');
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                echo "<h2>" . $result['row_count'] . "</h2>";
            } catch (PDOException $e) {
        ...
        <a href="?p=&amp;list_all_token=2" class="dashboard-card">
            <?php
            // Requête pour compter le nombre de lignes
            try {
                $stmt = $pdo->query('SELECT COUNT(*) AS row_count FROM fmh_token');
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                echo "<h2>" . $result['row_count'] . "</h2>";
            } catch (PDOException $e) {
                die("Erreur lors de l'exécution de la requête : " . $e->getMessage());
...
if (isset($_GET['admin_local_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    function get_active_local_sessions() {
        $session_path = ini_get('session.save_path');
        $active_sessions = [];
        ...
        if (is_dir($session_path)) {
            ...
            foreach ($files as $file) {
                ...
                if ($data) {
                    session_decode($data);
                    if (isset($_SESSION['logged']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===False) {
                        $active_sessions[] = $_SESSION['logged']; // Ajoutez l'identifiant utilisateur
                    }
                }
            }

            if (strlen($_SESSION['jail']) > 0){
                unset($_SESSION['jail']);
            }
    ...
    $active_users = get_active_local_sessions();
    ...
    <div class="container mt-5">
        <h1>Liste des utilisateurs locaux</h1>
        <br><br>
        <table class="table table-striped">
            ...
            <tbody>
            <?php foreach ($auth_users as $user => $password_hash): ?>
                <tr>
                    <td>
                        <?php echo htmlspecialchars($user); ?>
                        <?php if (in_array($user, $active_users)): ?>
                            <span style="color: green; font-size: 1.2em; margin-left: 5px;">●</span>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
...
if (isset($_GET['admin_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    <div class="container mt-5">
        ...
        <a href="<?php echo $basename; ?>?p=&admin_add_user=2" class="btn btn-success btn-sm"><i class="fa-solid fa-user-plus"></i></a>
        <a href="<?php echo $basename; ?>?p=&admin_logout_all_user=2" class="btn btn btn-dark btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir déconnecter tout le monde ?');"><i class="fa-solid fa-users-slash"></i></a>
        <br><br>
        <table class="table table-striped">
            <thead>
            <tr>
            ...
            <?php foreach ($users as $user): ?>
                <tr>
                    <td>
                        ...
                        <?php if (in_array($user['user_name'], $active_users)): ?>
                            <span style="color: green; font-size: 1.2em; margin-left: 5px;">●</span>
                        <?php endif; ?>
                    </td>
                    <td><?php echo htmlspecialchars($user['user_name']); ?></td>
                    <td><?php echo htmlspecialchars($user['user_role']); ?></td>
                    <td><?php echo $user['user_active'] ? 'Oui' : 'Non'; ?></td>
                    <td><?php echo htmlspecialchars($user['user_jail']); ?></td>
                    <td>
                        <a href="<?php echo $basename; ?>?p=&admin_edit_user=2&id=<?php echo $user['user_id']; ?>" class="btn btn-warning btn-sm"><i class="fa-solid fa-user-pen"></i></a>
                        <a href="<?php echo $basename; ?>?p=&admin_delete_user=2&id=<?php echo $user['user_id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?');"><i class="fa-solid fa-user-xmark"></i></a>
                        <?php if (in_array($user['user_name'], $active_users)): ?>
                            <a href="<?php echo $basename; ?>?p=&admin_logout_user&id=<?php echo $user['user_id']; ?>" class="btn btn-dark btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir déconnecter cet utilisateur ?');"><i class="fa-solid fa-user-slash"></i></a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
...
if (isset($_GET['get_token']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        ...
        function generateToken($length = 32) {
            return bin2hex(random_bytes($length / 2)); // Crée un token hexadécimal de 64 caractères
        }

        // Exemple d'ID utilisateur pour lequel le token sera généré
        $userId = $_SESSION['user_id']; // Par exemple, l'ID de l'utilisateur est 1

        // Générer le token pour l'utilisateur
        $token = generateToken();

        // Préparer et exécuter la requête d'insertion du token dans la base de données
        $stmt = $pdo->prepare("INSERT INTO fmh_token (user_id, user_name, token) VALUES (:user_id, :user_name, :token)");
        $stmt->execute([
            ':user_id' => $userId,
        ...
        // Retourner le token généré pour confirmation
        // echo "Le token généré pour l'utilisateur avec ID $userId est : $token\n";
        echo "<script>window.location.href='".$basename."?p=&list_token=2';</script>";
...
if (isset($_GET['list_token']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        ...
        // Récupérer les tokens pour cet utilisateur
        $stmt = $pdo->prepare("SELECT * FROM fmh_token WHERE user_id = :user_id");
        $stmt->execute([':user_id' => $userId]);
        $tokens = $stmt->fetchAll(PDO::FETCH_ASSOC);

       

        // Affichage des tokens dans une table HTML
        ?>
        <div class="container mt-5">
            <br>
            ...
            <div class="note-box">
                ...
                <div class="note-content">
                    ...
                    Pour upload des fichiers en ligne de commande, il suffit d'utiliser la commande suivante en précisant le token et le chemin vers le fichier<br><br>
                    <div class="code-block">
                        <?php 
                        echo 'curl -X POST -F "token=<b style="color: #c2352b;">YOUR_TOKEN</b>" -F "file=@<b style="color: #c2352b;">YOUR_FILE</b>" '. fm_enc(FM_ROOT_URL) .'/'. $basename .'?upload_cli=2'
                        ?>
                    </div>
                </div>
            </div>
            <a href="<?php echo $basename; ?>?p=&get_token=2" class="btn btn-success btn-sm"><i class="fa-solid fa-plus"></i></a>
            <br><br>
            <table class="table table-striped">
                <thead>
                    <tr>
...
if (isset($_GET['list_all_token']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        ...
        // Récupérer les tokens pour cet utilisateur
        $stmt = $pdo->prepare("SELECT * FROM fmh_token");
        $stmt->execute();
        $tokens = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ...
                <tbody>
                    <?php foreach ($tokens as $token): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($token['token']); ?></td>
                            <?php echo '<td><a href="'. $basename .'?p=&delete_token=2&token_id=' . $token['token_id'] . '" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer ce token ?\');">Supprimer</a></td>'; ?>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
if (isset($_GET['delete_token']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        ...
        // Vérifier si le token_id est passé via l'URL
        $tokenId = isset($_GET['token_id']) ? (int)$_GET['token_id'] : 0;

        if ($tokenId <= 0) {
            echo "Token ID invalide.";
            exit;
        }

        // Supprimer le token de la base de données
        $stmt = $pdo->prepare("DELETE FROM fmh_token WHERE token_id = :token_id");
    ...
    echo "<script>window.location.href='".$basename."?p=&list_all_token=2';</script>";
    exit;
}
...
if (isset($_GET['admin_add_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $role = trim($_POST['role']); // Optionnel : rôle de l'utilisateur
        $jail = trim($_POST['jail']); // Optionnel : jail de l'utilisateur
        ...
        if ($username && $password) {
            try {
                ...
                $stmt = $pdo->prepare('INSERT INTO fmh_users (user_name, user_pass, user_role, user_active, user_jail) VALUES (:username, :password, :role, :active, :jail)');
                $stmt->execute([
                    ':username' => $username,
                ...
                echo "<script>window.location.href='".$basename."?p=&admin_user=2';</script>";
            } catch (PDOException $e) {
    ...
    <div class="container mt-5">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <small id="generated-password" class="text-muted d-block mt-2"></small>
            </div>
            <div class="mb-3">
                ...
                <div>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="role" id="role_readonly" value="read_only" checked>
                        <label class="form-check-label" for="role_readonly">Read-Only</label>
                    </div>
                </div>
            </div>
            <div class="mb-3">
                <label for="jail" class="form-label">Jail (optionnel)</label>
                <input type="text" class="form-control" id="jail" name="jail">
            </div>
            <button type="submit" class="btn btn-primary">Ajouter</button>
        </form>
        <br>
        <a href="<?php echo $basename; ?>?p=&admin_user=2" class="btn btn-warning btn-sm">Retour</a>
    </div>
...
if (isset($_GET['admin_edit_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Préparez la requête SQL de base
            $query = "UPDATE fmh_users SET user_name = ?, user_role = ?, user_jail = ?, user_active = ?";
            $params = [$username, $role, $jail, $active];
        ...
        $stmt = $pdo->prepare("SELECT * FROM fmh_users WHERE user_id = ?");
        $stmt->execute([$id]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    <div class="container mt-5">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                <label class="form-label">Rôle</label>
                <div>
                    <?php $current_role = $user['user_role']; // Supposons que vous récupérez le rôle actuel ici ?>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="role" id="role_admin" value="admin" 
                            <?php echo $current_role === 'admin' ? 'checked' : ''; ?>>
                        <label class="form-check-label" for="role_admin">Admin</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="role" id="role_user" value="user" 
                            <?php echo $current_role === 'user' ? 'checked' : ''; ?>>
                        <label class="form-check-label" for="role_user">User</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="role" id="role_readonly" value="read_only" 
                            <?php echo $current_role === 'read_only' ? 'checked' : ''; ?>>
                        <label class="form-check-label" for="role_readonly">Read-Only</label>
                    </div>
                </div>
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="jail" name="jail" value="<?php echo htmlspecialchars($user['user_jail']); ?>">
            </div>
            <div class="mb-3">
                <label for="active" class="form-label">Actif</label>
                <input type="checkbox" id="active" name="active" <?php echo $user['user_active'] ? 'checked' : ''; ?>>
            </div>
...
if (isset($_GET['admin_delete_user']) && isset($_SESSION['db_user']) && $_SESSION['db_user']===True) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH, $_SESSION); // current path
    global $cfg, $lang;
?>
    ...
    try {
        $pdo = new PDO(DSN, DB_USER, DB_PASSWORD);
        $stmt = $pdo->prepare("DELETE FROM fmh_users WHERE user_id = ?");
        $stmt->execute([$id]);
        echo "<script>window.location.href='".$basename."?p=&admin_user=2';</script>";
        exit;
    } catch (PDOException $e) {
...
if (isset($_GET['view'])) {
    ...
    $file_url = FM_ROOT_URL . fm_convert_win((!isset($_SESSION['jail']) ? '/' . FM_ROOT_PATH : '') . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $file);
...
?>
    <div class="row">
        <div class="col-12">
            <ul class="list-group w-50 my-3" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                <?php if (!FM_READONLY): ?>
                    <a class="fw-bold btn btn-outline-primary" title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($file) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete') . ' ' . lng('File'); ?>','<?php echo urlencode($file); ?>', this.href);"> <i class="fa-solid fa-trash"></i> Delete</a>
                <?php endif; ?>
                <a class="fw-bold btn btn-outline-primary" href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="fa-solid fa-square-arrow-up-right"></i> <?php echo lng('Open') ?></a></b>
                <?php
                // ZIP actions
                ...
                ?>
                    <a class="fw-bold btn btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file">
                        <i class="fa-solid fa-square-pen"></i> <?php echo lng('Edit') ?>
                    </a>
                    <a class="fw-bold btn btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace"
                        class="edit-file"><i class="fa-solid fa-square-pen"></i> <?php echo lng('AdvancedEditor') ?>
                    </a>
                <?php } ?>
        ...
        <div class="row">
                ...
                <div class="btn-toolbar" role="toolbar">
                    <?php if (!$isNormalEditor) { ?>
                        <div class="btn-group js-ace-toolbar">
                            <button data-cmd="none" data-option="fullscreen" class="btn btn-sm btn-outline-secondary" id="js-ace-fullscreen" title="<?php echo lng('Fullscreen') ?>"><i class="fa-solid fa-up-right-and-down-left-from-center" title="<?php echo lng('Fullscreen') ?>"></i></button>
                            <button data-cmd="find" class="btn btn-sm btn-outline-secondary" id="js-ace-search" title="<?php echo lng('Search') ?>"><i class="fa-solid fa-magnifying-glass" title="<?php echo lng('Search') ?>"></i></button>
                            <button data-cmd="undo" class="btn btn-sm btn-outline-secondary" id="js-ace-undo" title="<?php echo lng('Undo') ?>"><i class="fa-solid fa-rotate-left" title="<?php echo lng('Undo') ?>"></i></button>
                            <button data-cmd="redo" class="btn btn-sm btn-outline-secondary" id="js-ace-redo" title="<?php echo lng('Redo') ?>"><i class="fa-solid fa-rotate-right" title="<?php echo lng('Redo') ?>"></i></button>
                            <button data-cmd="none" data-option="wrap" class="btn btn-sm btn-outline-secondary" id="js-ace-wordWrap" title="<?php echo lng('Word Wrap') ?>"><i class="fa-solid fa-text-width" title="<?php echo lng('Word Wrap') ?>"></i></button>
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                <div class="btn-group">
                    ...
                    <?php if ($is_text) { ?>
                        <?php if ($isNormalEditor) { ?>
                            <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa-solid fa-square-pen"></i> <?php echo lng('AdvancedEditor') ?></a>
...
?>
    <div class="path">
        <div class="card mb-2" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            ...
            <div class="card-body">
                ...
                <form action="" method="post">
                    ...
                    <table class="table compact-table" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                        <tr>
                            <td></td>
...
<form action="" method="post" class="pt-3">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            ...
            foreach ($folders as $f) {
                ...
                $img = $is_link ? 'icon-link_folder' : 'fa-solid fa-folder-open';
            ...
            ?>
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, '1028','<?php echo lng('Delete') . ' ' . lng('Folder'); ?>','<?php echo urlencode($f) ?>', this.href);"> <i class="fa-solid fa-trash-can" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa-solid fa-square-pen" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa-solid fa-copy" aria-hidden="true"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (!isset($_SESSION['jail']) ? '/' . FM_ROOT_PATH : '') . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa-solid fa-link" aria-hidden="true"></i></a>
                    </td>
                </tr>
            <?php
                flush();
            ...
            foreach ($files as $f) {
                $is_link = is_link($path . '/' . $f);
                $img = $is_link ? 'fa-regular fa-file-lines' : fm_get_file_icon_class($path . '/' . $f);
                $modif_raw = filemtime($path . '/' . $f);
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                $date_sorting = strtotime(date("F d Y H:i:s.", $modif_raw));
            ...
            ?>
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete') . ' ' . lng('File'); ?>','<?php echo urlencode($f); ?>', this.href);"> <i class="fa-solid fa-trash-can"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa-solid fa-square-pen"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..."
                                href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa-solid fa-copy"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (!isset($_SESSION['jail']) ? '/' . FM_ROOT_PATH : '') . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa-solid fa-link"></i></a>
                        <a title="<?php echo lng('Download') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1211, '<?php echo lng('Download'); ?>','<?php echo urlencode($f); ?>', this.href);"><i class="fa-solid fa-download"></i></a>
                    </td>
                </tr>
            <?php
                flush();
                    ...
                    <tr>
                        <td class="gray fs-7" colspan="<?php echo (!FM_IS_WIN && !$hide_Cols) ? (FM_READONLY ? '6' : '7') : (FM_READONLY ? '4' : '5') ?>">
                            ...
                            <?php
                                ...
                                // Obtenir la taille totale de la partition
                                $totalSpace = disk_total_space($path);

                                //Obtenir la taille utilisée
                                $usedSpace = $totalSpace - $freeSpace;

                                // Convertir en unités lisibles (par exemple, en Go)
                                $usedSpaceGB = $usedSpace / (1024 * 1024 * 1024);
                                $totalSpaceGB = $totalSpace / (1024 * 1024 * 1024);
                            ?>
                            <?php echo lng('DiskSpace') . ': <span class="badge text-bg-light border-radius-0">' . round($usedSpaceGB, 2) . '/' . round($totalSpaceGB, 2) . ' Go' . '</span>' ?>
                        </td>
                    </tr>
                </tfoot>
            <?php } ?>
        ...
        <?php if (!FM_READONLY): ?>
            <div class="col-xs-12 col-sm-9">
                <div class="btn-group flex-wrap" data-toggle="buttons" role="toolbar">
                    ...
                    <a href="#/invert-all" class="btn btn-small btn-outline-primary btn-2" onclick="invert_all();return false;"><i class="fa-solid fa-table-list"></i> <?php echo lng('InvertSelection') ?> </a>
                    ...
                    <a href="javascript:document.getElementById('a-zip').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa-regular fa-file-zipper"></i> <?php echo lng('Zip') ?> </a>
                    <input type="submit" class="hidden" name="tar" id="a-tar" value="tar" onclick="return confirm('<?php echo lng('Create archive?'); ?>')">
                    <a href="javascript:document.getElementById('a-tar').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa-regular fa-file-zipper"></i> <?php echo lng('Tar') ?> </a>
...
function fm_get_file_icon_class($path)
{
    ...
    switch ($ext) {
        ...
        case 'htpasswd':
        case 'pl':
            $img = 'fa-regular fa-file-lines';
            break;
        case 'css':
        case 'less':
        case 'sass':
        case 'scss':
            $img = 'fa-brands fa-css3';
            break;
        case 'bz2':
        case 'tbz2':
        case 'tbz':
        ...
        case 'docx':
        case 'odt':
            $img = 'fa-regular fa-file-word';
            break;
        case 'ppt':
        case 'pptx':
            $img = 'fa-regular fa-file-powerpoint';
            break;
        case 'ttf':
        case 'ttc':
        case 'otf':
 ...
 * @param string $path
 */
function fm_show_nav_path($path, $session)
{
    global $lang, $sticky_navbar, $editFile;
    $isStickyNavBar = $sticky_navbar ? 'fixed-top' : '';
?>
    <nav class="navbar navbar-expand-lg mb-4 main-nav <?php echo $isStickyNavBar ?> bg-body-tertiary" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                    ...
                    <?php if (FM_USE_AUTH): ?>
                        <li class="nav-item avatar dropdown">
                            <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="fa-solid fa-circle-user"></i> <?php echo $_SESSION['logged'] ?>
                            </a>
                            <div class="dropdown-menu dropdown-menu-end text-small shadow" aria-labelledby="navbarDropdownMenuLink-5" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                                <?php
                                if (isset($_SESSION['role']) && $_SESSION['role'] === "admin"){
                                    echo '<a title="Administration" class="dropdown-item nav-link" href="?p=&amp;admin_page=2"><i class="fa-solid fa-hammer" aria-hidden="true"></i> Administration</a>';
                                }
                                
                                if (isset($_SESSION['db_user']) && $_SESSION['db_user']===True){
                                    echo '<a title="Get token" class="dropdown-item nav-link" href="?p=&amp;list_token=2"><i class="fa-solid fa-square-binary" aria-hidden="true"></i> Get token</a>';
                                }

                                if ($GLOBALS['config']['FM_THEME'] === 'light'){
                                    echo '<a title="Change theme" class="dropdown-item nav-link" id="theme-toggle"><i class="fa-regular fa-sun" aria-hidden="true"></i> Light Mode</a>';
                                }else{
                                    echo '<a title="Change theme" class="dropdown-item nav-link" id="theme-toggle"><i class="fa-solid fa-moon" aria-hidden="true"></i> Dark Mode</a>';
                                }

                                if (isset($_SESSION['db_user']) && $_SESSION['db_user']===True){
                                    echo '<a title="Change password" class="dropdown-item nav-link" href="?p=&amp;passwd_user=2"><i class="fa-solid fa-key" aria-hidden="true"></i> Change password</a>';
                                }
                                ?>
...
?>
    <!DOCTYPE html>
    <html lang="en" data-bs-theme="<?php echo ($GLOBALS['config']['FM_THEME'] == "dark") ? 'dark' : 'light' ?>">
    ...
    <body class="fm-login-page <?php echo ($GLOBALS['config']['FM_THEME'] == "dark") ? 'theme-dark' : ''; ?>">
...
?>
    <!DOCTYPE html>
    <html data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
    ...
    <head>
        ...
        <style>
            ...
            *::after {
            ...
            .note-box {
                ...
                border-left: 5px solid #547d93; /* Couleur de la barre gauche */
                padding: 10px;
            ...
            .note-icon {
                flex-shrink: 0;
                font-size: 24px;
                color: #547d93; /* Couleur de l'icône */
            ...
            .list-group .list-group-item-active-bg {
                color: #535c66;
            }
        ...
        <?php
        if ($GLOBALS['config']['FM_THEME'] == "dark"): ?>
            <style>
                :root {
                    --bs-bg-opacity: 1;
                    --bg-color: #f3daa6;
                ...
                .list-group .list-group-item .activr {
                    background: #535c66;
                }
    ...
    <body class="<?php echo ($GLOBALS['config']['FM_THEME'] == "dark") ? 'theme-dark' : ''; ?> <?php echo $isStickyNavBar; ?>">
        <div id="wrapper" class="container-fluid">
            <!-- New Item creation -->
            <div class="modal fade" id="createNewItem" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="newItemModalLabel" aria-hidden="true" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            ...
            <div class="modal fade" id="searchModal" tabindex="-1" role="dialog" aria-labelledby="searchModalLabel" aria-hidden="true" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
            ...
            <!--Rename Modal -->
            <div class="modal modal-alert" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" id="renameDailog" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                <div class="modal-dialog" role="document">
            ...
            <script type="text/html" id="js-tpl-confirm">
                <div class="modal modal-alert confirmDailog" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" id="confirmDailog-<%this.id%>" data-bs-theme="<?php echo $GLOBALS['config']['FM_THEME']; ?>">
                    <div class="modal-dialog" role="document">
    ...
    function fm_show_footer()
    {
        ...
        <script>
            document.getElementById('generate-password').addEventListener('click', function() {
                ...
                // Remplir le champ mot de passe avec le mot de passe généré
                const passwordField = document.getElementById('password');
                passwordField.value = randomPassword;

                // Afficher le mot de passe généré en dessous du champ
                const generatedPasswordDisplay = document.getElementById('generated-password');
                generatedPasswordDisplay.textContent = 'Mot de passe généré : ' + randomPassword;
            });
            ...
            // Appliquer le thème en fonction du cookie
            function applyTheme(theme) {
                if (theme === 'dark'){
                    <?php $GLOBALS['config']['FM_THEME'] = 'dark';?>
                }else{
                    <?php $GLOBALS['config']['FM_THEME'] = 'light';?>
                }
            }
            ...
            document.getElementById('theme-toggle').addEventListener('click', () => {
                ...
                setCookie('theme', newTheme); // Stocker le thème dans un cookie pour 7 jours
                applyTheme(newTheme);
                location.reload();
            });
        </script>
...
<?php
    ...
    function lng($txt)
    {
        ...
        // English Language
        $tr['en']['AppName']        = 'File Manager Hosting';
